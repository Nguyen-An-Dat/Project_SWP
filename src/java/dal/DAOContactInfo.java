...
public class DAOContactInfo extends DBContext {
    ...
    public List<listInfoCus> getAllCus() {
        List<listInfoCus> list1 = new ArrayList<>();
        String sql = "select * from listInfoCus";

        try {
            PreparedStatement st = connection.prepareStatement(sql);
            ResultSet rs = st.executeQuery();
            while (rs.next()) {
                listInfoCus c = new listInfoCus(
                        rs.getInt("Id"),
